...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        if (!annotations) {
            EV_WARN << "DemoBaseApplLayer: AnnotationManager not found\n";
        }

        mac = FindModule<DemoBaseApplLayerToMac1609_4Interface*>::findSubModule(getParentModule());
        if (!mac) {
            EV_WARN << "DemoBaseApplLayer: MAC interface not found in "
        ...
        if (!mac) {
            EV_WARN << "DemoBaseApplLayer: skipping stage-1 MAC setup for "
                    << getParentModule()->getFullPath() << "\n";
            return;
        }
